...
(gallery at published v80), G9 (zip fresh), G18 (accuracy sweep clean); the
...
| G5 | Screenshot gallery refreshed with launch-build captures (current = **v80 dev build** — refreshed v166 — + v16 interior vignettes + v1 early-pass pair; gonogo.sh flags future deltas automatically) | mkt, needs art publish | `[x] REHEARSED` — swap procedure executed end-to-end 2026-09-23; repeat at launch if art publishes newer |
...
| T-4d | Human playtest pass (G17): run `world/playtest.html` PT1–PT104 on the launch candidate; export blocker/major findings via "Copy inbox note"; owner dispositions every major | G17 |
...
| Human playtest (G17) | | | 0 open blockers on launch candidate · PT1–PT104 |
...
| G17 human playtest | `world/playtest.html` harness (live: PT1–PT104 (checkpoint count grows with the harness — cite the live count), `#pt=` deep links, triage export); owner run on launch candidate owed | harness shipped; run owed |
