...
(gallery at published v81), G9 (zip fresh), G18 (accuracy sweep clean); the
...
| G5 | Screenshot gallery refreshed with launch-build captures (current = **v81 dev build** — refreshed v167 — + v16 interior vignettes + v1 early-pass pair; gonogo.sh flags future deltas automatically) | mkt, needs art publish | `[x] REHEARSED` — swap procedure executed end-to-end 2026-09-23; repeat at launch if art publishes newer |
...
| G17 | Human playtest: owner (or designate) runs the live playtest harness end-to-end on the near-launch build (`world/playtest.html`, PT1–PT112 (checkpoint count grows with the harness — cite the live count), `#pt=` deep links, harness keyboard map, per-scenario timing in `session.time_per_scenario`). Blocker/major findings exported via "Copy inbox note" into the shared inbox; **zero open blockers and every major triaged with an owner-visible disposition** before GO. This is the only gate that proves a human can actually get through the front door — every other gate proves a mechanism | owner + world track | `[ ] PENDING` — harness pinned live by checklist_audit (PT1–PT112); run owed on the launch candidate build |
...
| T-4d | Human playtest pass (G17): run `world/playtest.html` PT1–PT112 on the launch candidate; export blocker/major findings via "Copy inbox note"; owner dispositions every major | G17 |
...
| Human playtest (G17) | | | 0 open blockers on launch candidate · PT1–PT112 |
...
| G17 human playtest | `world/playtest.html` harness (live: PT1–PT112 (checkpoint count grows with the harness — cite the live count), `#pt=` deep links, triage export); owner run on launch candidate owed | harness shipped; run owed |
