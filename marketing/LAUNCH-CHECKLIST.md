...
| G17 | Human playtest: owner (or designate) runs the world-v65 playtest harness end-to-end on the near-launch build (`world/playtest.html`, PT1–PT60 · 729 checkpoints, `#pt=` deep links, harness keyboard map, per-scenario timing in `session.time_per_scenario`). Blocker/major findings exported via "Copy inbox note" into the shared inbox; **zero open blockers and every major triaged with an owner-visible disposition** before GO. This is the only gate that proves a human can actually get through the front door — every other gate proves a mechanism | owner + world track | `[ ] PENDING` — harness now at world-v65; run owed on the launch candidate build |
...
| T-4d | Human playtest pass (G17): run `world/playtest.html` PT1–PT60 on the launch candidate; export blocker/major findings via "Copy inbox note"; owner dispositions every major | G17 |
...
| Human playtest (G17) | | | 0 open blockers on launch candidate · PT1–PT60 |
...
| G17 human playtest | `world/playtest.html` harness (world-v65: PT1–PT60 · 729 checkpoints, `#pt=` deep links, triage export); owner run on launch candidate owed | harness shipped; run owed |
