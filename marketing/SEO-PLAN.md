...
|---|---|---|---|---|
| `/` (index.html) | Real World — A Living Neighborhood You Can Watch | truman show game, AI life sim | ✔ VideoGame | Hero `v43-D` preloaded (webp, fetchpriority=high); #wire feed-strip anchor — "the wire" copy is index-owned, demo.html owns "watch" queries |
...
one PNG fallback (v43-C, 1.9MB) is under the 2MB ceiling — accepted, it only
...
1. `shots/v43-C.png` is 1.9MB and `v43-A.png` is 1.8MB (accepted — webp
