...
|---|---|---|---|---|
| `/` (index.html) | Real World — A Living Neighborhood You Can Watch | truman show game, AI life sim | ✔ VideoGame | Hero `v32-D` preloaded (webp, fetchpriority=high); #wire feed-strip anchor — "the wire" copy is index-owned, demo.html owns "watch" queries |
...
one PNG fallback (v32-C, 1.9MB) is under the 2MB ceiling — accepted, it only
...
- Asset hooks that earn embeds: before/after v1→v32 gallery, the public request
...
1. `shots/v32-C.png` is 1.9MB and `v32-A.png` is 1.8MB (accepted — webp
