...
- Real development capture (`shots/v44-A.*`) behind a gradient overlay labeled
...
- **Rotating fallback captures** — while `data-demo-src` is empty, the
  fallback screen cycles v44-A–D every 8 s (crossfade) with a `.demo-cap`
...
- **Art:** fallback capture is `shots/v44-A.*`; swap when a better canonical
