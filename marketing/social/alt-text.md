# Alt-text bank — every social asset, copy-paste ready

Rule: every image post carries alt text. Describe the shot literally,
then the one thing that matters. Never "screenshot of a game." Keep
under ~125 chars where the platform warns; the long version is for the
site, the short for X/Bluesky/Mastodon.

## Published shots (`site/shots/`, development build v81 current; v80/v76/v75/v71/v65/v61/v59/v55/v54/v53 kept for era posts)

| File | Alt text |
|---|---|
| `v81-A` | Top-down view of the Mission block with the Mudhaus Coffee dollhouse cutaway open — Jules inside the drawn floor plan, Marcus and Victor at the door, the rooftop-over-park camera feed live in the corner monitor. Development build. |
| `v81-B` | Street-level close view of Jules on the sidewalk with Priya and Dani mid-block — dressed facades, parked cars along the curb, leaves drifting. Development build. |
| `v81-C` | Dolores Park from overhead — crown-genome trees in rust and gold, meadow drifts across the lawns, building-scale shade pooling on the east edge, tennis courts holding the south edge. Development build. |
| `v81-D` | Director-mode street view with the thirds grid and center cross up — dressed fronts and rooftops below a clean sky, REC cluster and DIRECTOR badge, viewfinder strip reading EV and compass. Development build. |
| `v80-A`–`v80-D` | The shade-knows-the-sky build — sky-visibility field, building-scale shade pools, self-shadowing props. Era set; v81 is the current build. |
...
| `v76-A`–`v76-D` | The crepuscular-lanes build — clouds throwing warm volumetric streaks to their sun-gaps, silver-lining rim arcs on every dome. Kept for era posts; v81 is the current build. |
| `v75-A`–`v75-D` | The living-walls build — ivy and bougainvillea fronts, meadow stipple drifts, the traveling gust field. Kept for era posts; v81 is the current build. |
| `v71-A`–`v71-D` | The dollhouse-and-shadows build — buildings throwing real swept shadows, the Mudhaus cutaway, the re-framed park overlook. Kept for era posts; v81 is the current build. |
...
| `v65-D` | Director-mode street view over rooftops toward distant cumulus shower cells on the skyline — REC cluster and DIRECTOR badge, wind-leaned rain shafts. Development build. |
| `v61-A`–`v61-D` | The weather-you-can-see-coming build — patchy Karl, distant shower cells with wind-leaned rain shafts, fog-drip wetting. Era set (individual alts below). |
| `v61-A` | Top-down view of the Mission block under patchy Karl fog — autumn crowns turning, streets gridded, the resident inspector open on Jules between things on Guerrero. Development build. |
...
| `v59-A`–`v59-D` | The wet-after-rain build — speckled damp pavement, ponding on flat roofs, stringcourses catching light at every floor line. Kept for era posts; v81 is the current build. |
...
| `v54-A`–`v54-D` | The facade-pass build — window boxes, AC sleeves, Juliet rails, house numbers, stoop pots on every frontage. Kept for era/before-after posts; v81 is the current build. |
...
| Asset | Alt text |
|---|---|
| `assets/logo-icon.png` | Real World logo mark — a row of simplified gabled rooftops. |
| `assets/keyart-16x9.png` | Key art: the whole Mission block at dusk, viewed from above the rooftops. |
| `assets/og-card.png` | Real World — twenty-eight lives on a real Mission block. Watch free. |
| Feed screenshots (live) | "The Wire — Real World's public feed. A request from viewer {{handle}} is shown {{status}}; a housing notice posts neutral wording." — template; rewrite to match what's actually on screen. Never post a feed shot that contains a slur/abuse attempt unmoderated — crop or wait for the resolved state. |
