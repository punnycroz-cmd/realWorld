...
//   NODE_PATH=/home/hatch/.npm/_npx/e41f203b7505f1fb/node_modules node devtools/shoot_v74.js
...
(async () => {
  ...
  await pg.evaluate(PIN + `;(() => {
    ...
    v.x = p.x; v.y = p.y;
    v.inBuilding = true; v.inside = 'Haus Coffee';   // v74 dollhouse
    v.moving = false;
  ...
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v74-A.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v74-B.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v74-C.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v74-D.png' });
