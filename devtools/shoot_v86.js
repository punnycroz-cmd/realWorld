// V85 canonical screenshots — Node port of shoot_v63.js (no py-playwright
// on this box). Same four framings, same pins. Run:
//   NODE_PATH=/home/hatch/.npm/_npx/e41f203b7505f1fb/node_modules node devtools/shoot_v86.js
...
(async () => {
  ...
  await pg.evaluate(PIN + `;(() => {
    ...
    v.x = p.x; v.y = p.y;
    v.inBuilding = true; v.inside = 'Haus Coffee';   // v86 dollhouse
    v.moving = false;
    const s1 = VILLAGERS[(inspectedPawnIdx + 3) % VILLAGERS.length];
    const s2 = VILLAGERS[(inspectedPawnIdx + 5) % VILLAGERS.length];
    s1.inBuilding = true; s1.inside = 'Haus Coffee'; s1.moving = false;
    s2.inBuilding = true; s2.inside = 'Haus Coffee'; s2.moving = false;
    cam.x = b.x; cam.y = b.y; cam.zoom = 0.85;
  })()`);
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ timeout: 90000, path: OUT + 'v86-A.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ timeout: 90000, path: OUT + 'v86-B.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ timeout: 90000, path: OUT + 'v86-C.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ timeout: 90000, path: OUT + 'v86-D.png' });
