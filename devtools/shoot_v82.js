// V80 canonical screenshots — Node port of shoot_v63.js (no py-playwright
// on this box). Same four framings, same pins. Run:
//   NODE_PATH=/home/hatch/.npm/_npx/e41f203b7505f1fb/node_modules node devtools/shoot_v82.js
...
(async () => {
  ...
  await pg.evaluate(PIN + `;(() => {
    ...
    v.x = p.x; v.y = p.y;
    v.inBuilding = true; v.inside = 'Haus Coffee';   // v82 dollhouse
    v.moving = false;
  ...
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v82-A.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v82-B.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v82-C.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v82-D.png' });
