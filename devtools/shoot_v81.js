...
//   NODE_PATH=/home/hatch/.npm/_npx/e41f203b7505f1fb/node_modules node devtools/shoot_v81.js
...
(async () => {
  ...
  await pg.evaluate(PIN + `;(() => {
    ...
    v.x = p.x; v.y = p.y;
    v.inBuilding = true; v.inside = 'Haus Coffee';   // v81 dollhouse
    v.moving = false;
  ...
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v81-A.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v81-B.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v81-C.png' });
  ...
  await pg.waitForTimeout(600);
  await pg.evaluate(PIN);
  await pg.screenshot({ path: OUT + 'v81-D.png' });
