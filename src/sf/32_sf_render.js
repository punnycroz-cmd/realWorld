...
function sfKarlK(){
  ...
  const diur = clamp(0.12 + 0.85 * Math.max(
    ...
    clamp((t - 15.2) / 3.4, 0, 1)), 0, 1);         // evening push
...
function sfRenderWorld(cw, ch){
  ...
  for(const d of drawables){
    if(d.kind === 'bld'){
      ...
      /* integration-1: label text comes from the canonical display-name
         layer (parody names + hidden kinds) — raw OSM names never render */
  ...
  if(karlK > 0.04 && !isNight()){
    const kpoly = sfKarlPoly(karlK * 0.32);
    ...
    if(kpoly.length >= 3 && kfront){
      ...
      const kg = ctx.createLinearGradient(up[0], up[1],
                                          fmx + (fmx - up[0]) * 0.25,
                                          fmy + (fmy - up[1]) * 0.25);
      kg.addColorStop(0, `rgba(228,234,240,${0.12 + karlK * 0.38})`);
      kg.addColorStop(0.75, `rgba(228,234,240,${0.06 + karlK * 0.22})`);
      ...
      for(let k = 0; k < nb; k++){
        ...
        const rr = (46 + phash(k, 74, 3863) * 60) * (0.6 + cam.zoom);
        const rg = ctx.createRadialGradient(bx, by, 0, bx, by, rr);
        rg.addColorStop(0, `rgba(230,236,242,${karlK * 0.3})`);
...
function sfRenderStreet(cw, ch){
  ...
  if(!night){
    ...
    // sun at golden hour.
    {
      ...
      const karlA = sfKarlK() * (0.2 + 0.8 * upFace);
      if(karlA > 0.04 && W.rain < 0.7){
        const wallH = fh * (0.6 + karlA * 2.4) + 8;
        const kg2 = ctx.createLinearGradient(0, horizon - wallH, 0, horizon + 34);
        kg2.addColorStop(0, `rgba(232,236,242,${0.45 * karlA})`);
        kg2.addColorStop(0.8, `rgba(208,218,230,${0.85 * karlA})`);
        kg2.addColorStop(1, `rgba(198,210,222,${0.55 * karlA})`);
        ctx.fillStyle = kg2;
        ctx.beginPath();
  ...
  for(const d of ds){
    ...
    if(d.k === 'b'){
      ...
      /* integration-1: floating label text via canonical display names —
         unnamed/hidden-kind buildings get no label (world rule) */
      const dn = sfDisplayName(b.name, b.kind);
      if(dn){
...
function sfRenderInterior(cw, ch, v){
  ...
  // location card — integration-1: parody display name (real interior
  // keys like 'Haus Coffee'/'744 Guerrero' map to canonical fiction)
