...
if(SF_MODE && typeof window !== 'undefined'){
  ...
  window.addEventListener('wheel', e => {
    ...
    // follow distance in third-person mode
    if(SF_CAM.director)
...
function sfRenderWorld(cw, ch){
  ...
  for(let wy = wy0; wy <= wy1; wy++){
    for(let wx = wx0; wx <= wx1; wx++){
      ...
      const sy = Math.round(sfSY(wy * CS, ch));
      if(spr && spr.c) ctx.drawImage(spr.c, sx, sy, cs, csT);
      ...
      if(sfDappleOn){
        ...
        if(a2 > 0.015){
          ...
          ctx.fillRect(sx - 1, sy - 1, cs + 2, csT + 2);
        }
      }
    }
  }
...
function sfRenderStreet(cw, ch){
  ...
  const camH = SF_CAM.h;
  let camX, camY, pitchEff = SF_CAM.pitch;
  if(SF_CAM.director){
    camX = SF_CAM.x; camY = SF_CAM.y;
  } else {
    const px = v ? v.x / SF_PXM : 600, py = v ? v.y / SF_PXM : 400;
    camX = px - DX * SF_CAM.back;
    camY = py - DY * SF_CAM.back;
    pitchEff = SF_CAM.pitch - Math.atan2(Math.max(0, camH - 1.05), Math.max(2, SF_CAM.back)) * 0.85;
  }
  const F = Math.max(400, ch * 1.1);
  const horizon = ch * 0.42 + Math.tan(pitchEff) * F;
  const pr = (x, y, z) => {
