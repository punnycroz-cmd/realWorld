      ...
      if(sprC){
        ...
        if(o.kind === 'sfTree' && sfTreeTurns(o) && !isNight() &&
          ...
          for(let fi = 0; fi < nF; fi++){
            ...
            const fx = sx + (phash(fi, o.wx + o.wy, 5532) - 0.5) * pw * 0.7 +
                       wdx2 * ph2 * 30 * cam.zoom +
                       Math.sin(ph2 * 11 + fi * 2.3) * 3.5 * cam.zoom;
            const fy = sy - ph * 0.55 + ph2 * (ph * 0.5 + 8 * cam.zoom) +
...
function sfRenderStreet(cw, ch){
  ...
  if(!night){
    ...
    // v7+v50: crepuscular sun shafts — the v7 hard arc wedges read as
    // glitch triangles whenever the disc sat off-frame (art-feedback).
    // v50 rebuilds them as volumetric light LANES: each lane is a chain
    // of overlapping radial-gradient billows descending along its ray,
    // warm late-afternoon tone, thinning with distance from the disc.
    // Every boundary is a gradient falloff — no polygon edges at all.
    if(sunFwd > 0.15 && W.rain < 0.4 && cover < 0.8){
      const sx3 = cw / 2 + (sunSide / Math.max(0.4, sunFwd)) * F * 0.9;
      const sy3 = horizon - sunUp * F * 0.72;
      // v47 physics kept: sunbreak lanes overhead are real gaps in the
      // deck, so lane strength follows cover and drifts with the gaps
      const shaftA = 0.16 * (1 - cover * 0.55);
      for(let k = 0; k < 7; k++){
        const ang = 0.42 + k * 0.24 + Math.sin(SF_WX.t * 0.2 + k) * 0.03;
        const dx = Math.cos(ang), dy = Math.sin(ang);
        const laneA = shaftA * (0.45 + phash(k, 96, 5301) * 0.75);
        for(let s = 0; s < 7; s++){
          const t = 0.10 + s * 0.15;                  // fraction along the ray
          const bx = sx3 + dx * F * 1.1 * t,
                by = sy3 + dy * F * 1.1 * t;
          if(bx < -F || bx > cw + F || by > ch + 40) break;
          const rad = (0.020 + t * 0.085) * F *
                      (0.75 + phash(k * 7 + s, 97, 5302) * 0.5);
          const a = laneA * (1 - t * 0.72);
          ...
          ctx.fillStyle = g;
          ctx.beginPath();
          ctx.ellipse(bx, by, rad, rad * 0.42, ang, 0, Math.PI * 2);
          ctx.fill();
        }
      }
    }
