...
/* ---------------- v26: LENS — camera post-processing rig -------------
   ...
   · both      — lens vignette + a whisper of film grain + lateral
     chromatic aberration (v62: red/cyan plates split and re-added).
...
function sfLensEnd(cw, ch){
  ...
  if(SF_LENS.roll && Math.abs(SF_LENS.roll) > 0.0004){
    ...
    /* v62: lateral chromatic aberration — a real lens disperses the
       channels: the red plate lands shifted one way, the cyan (G+B)
       plate the other, so high-contrast silhouettes fringe warm/cool
       instead of staying surgically clean. The frame is split by
       multiply masks and re-added with 'lighter'; where the plates
       overlap the channels recombine to the original color. */
    if(SF_LENS.mode !== 'flat' && typeof document !== 'undefined'){
      if(!SF_LENS.caR){
        SF_LENS.caR = document.createElement('canvas');
        SF_LENS.caRg = SF_LENS.caR.getContext('2d');
        SF_LENS.caB = document.createElement('canvas');
        SF_LENS.caBg = SF_LENS.caB.getContext('2d');
      }
      const R2 = SF_LENS.caR, rg = SF_LENS.caRg,
            B2 = SF_LENS.caB, b2g = SF_LENS.caBg;
      if(R2.width !== W || R2.height !== H){
        R2.width = W; R2.height = H; B2.width = W; B2.height = H;
      }
      const d2 = Math.max(1, Math.round(W * 0.0011));
      rg.setTransform(1, 0, 0, 1, 0, 0);
      rg.clearRect(0, 0, W, H);
      rg.drawImage(F, d2, 0);
      rg.globalCompositeOperation = 'multiply';
      rg.fillStyle = '#ff0000'; rg.fillRect(0, 0, W, H);
      // multiply flood-fills the transparent border — clip back to the
      // plate's real alpha before recombining
      rg.globalCompositeOperation = 'destination-in';
      rg.drawImage(F, 0, 0);
      rg.globalCompositeOperation = 'source-over';
      b2g.setTransform(1, 0, 0, 1, 0, 0);
      b2g.clearRect(0, 0, W, H);
      b2g.drawImage(F, -d2, 0);
      b2g.globalCompositeOperation = 'multiply';
      b2g.fillStyle = '#00ffff'; b2g.fillRect(0, 0, W, H);
      b2g.globalCompositeOperation = 'destination-in';
      b2g.drawImage(F, 0, 0);
      b2g.globalCompositeOperation = 'source-over';
