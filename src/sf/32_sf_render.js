...
const SF_CAM = {
  ...
  guides: true,      // v81: director frame guides (thirds grid + cross), H toggles
  _lastPawn: -1,
...
window.addEventListener('keydown', e => {
  ...
  // v81: H toggles the director's frame guides (thirds grid + center cross)
  if(e.code === 'KeyH' && SF_VIEW === 'street'){
    SF_CAM.guides = SF_CAM.guides === false;
    if(typeof showToast === 'function')
      showToast(SF_CAM.guides ? '▦ GUIDES — thirds grid on' : 'Guides off');
  }
...
function sfLensEnd(cw, ch){
  ...
  /* v84: the grade — the last thing light does is meet the eye. Every
     pass before this painted light locally; nothing balanced the frame
     as a whole. A gentle S-curve + saturation lift on the frame plate
     keeps sun-struck pigment warm and lets skylit shade fall to cool
     depth instead of neutral milk — the two-source daylight the scene
     already computes, now readable at a glance. Strength rides the
     real sun: overcast grades flatter, golden hour grades richer,
     night barely touches. Applied ONLY to the world plate so the
     optical stack after it (bokeh mask, ghosts, glare, droplets) and
     the HUD stay ungraded chrome. */
  {
    const grK = (typeof isNight === 'function' && isNight()) ? 0.3
              : (typeof SF_SUN !== 'undefined'
                 ? 0.5 + SF_SUN.day * 0.5 : 0.5);
    const warmK = typeof SF_SUN !== 'undefined' ? SF_SUN.warm : 0;
    if(typeof ctx.filter === 'string')
      ctx.filter = `contrast(${(1 + 0.20 * grK).toFixed(3)}) ` +
                   `saturate(${(1 + 0.30 * grK).toFixed(3)}) ` +
                   `brightness(${(1 + 0.02 * grK).toFixed(3)}) ` +
                   `sepia(${(0.10 * warmK).toFixed(3)})`;
  }
  ...
  ctx.filter = 'none';   // v84: the grade ends where the world plate ends
     ...
     downward at droplet speeds while it's actually raining. */
  {
    const rainK = (SF_LENS.mode === 'street' && typeof W !== 'undefined')
      ? clamp(W.rain, 0, 1) : 0;
    if(rainK > 0.12){
      const nD = Math.floor((6 + rainK * 14) * sfQualF());
      /* droplet bead pre-rendered once — a radial gradient per droplet
         per frame was the classic canvas anti-pattern; blit the sprite
         instead (alpha comes from globalAlpha, same look) */
      if(!SF_LENS.dropSpr){
        const s = document.createElement('canvas'); s.width = s.height = 64;
        const sg = s.getContext('2d');
        const gr = sg.createRadialGradient(32, 32, 6, 32, 32, 32);
        gr.addColorStop(0, 'rgba(200,215,230,0.7)');
        gr.addColorStop(0.75, 'rgba(30,40,55,0.35)');
        gr.addColorStop(1, 'rgba(30,40,55,0)');
      ...
      for(let i = 0; i < nD; i++){
        ...
        ctx.drawImage(SF_LENS.dropSpr, rx - rr, ry - rr, rr * 2, rr * 2);
        ctx.globalAlpha = 1;
...
const SF_HAZE_MEMO = { fr: -1, m: new Map() };   // v82: haze per 2m depth
function sfHazeA(fwd){
  const k = Math.round(fwd * 0.5);
  if(SF_HAZE_MEMO.fr !== SF_FRAME){ SF_HAZE_MEMO.m.clear(); SF_HAZE_MEMO.fr = SF_FRAME; }
  const hit = SF_HAZE_MEMO.m.get(k);
  if(hit !== undefined) return hit;
  const v = Math.min(0.62, Math.max(0, (fwd - 45) / 240) * SF_WX.hazeK);
  SF_HAZE_MEMO.m.set(k, v);
  return v;
}
...
function sfSunWallCol(base, k){
  const lit = 0.56 + 0.55 * Math.max(0, k) * SF_SUN.day + 0.05 * Math.max(0, k);
  if(SF_SUN.day > 0.22 && k > 0.18)
    return mix(shade(base, lit), '#ffd98e', SF_SUN.warm * 0.68 * Math.min(1, k));
  return mix(shade(base, lit), '#6680b2', 0.30 * (1 - Math.max(0, k) * SF_SUN.day));
}
...
function sfCanyonShade(xm, ym, selfI){
  ...
  /* v82: the march is memoized per 4m cell per occluding self — the shade
     height it returns varies on facade scale, so a cell far under a
     parapet shadow length is exact-enough and identical per frame */
  const k = (selfI + 2) * 1e6 + Math.round(xm * 0.25) * 1024 +
            Math.round(ym * 0.25);
  const hit = sfLFGet('cany', k);
  if(hit !== undefined) return hit;
...
/* ---------------- v80: the sun field --------------------------------
   Every pass before this painted shadows LOCALLY — a building's bake can
   only shade inside its own canvas pad, so a 20m rowhouse never throws
   its real 40m evening shadow across the street, and nothing shades the
   park. This replaces the patchwork with a sky-visibility FIELD: for
   each half-cell ground sample we march a ray toward the sun through
   building masses AND crown volumes (elevations are absolute, so the
   Dolores bowl and Liberty Hill carry shade uphill/downhill correctly);
   the result bakes into the terrain chunk as a cool skylight-only shade
   layer plus a visibility-gated warm key. Crowns transmit a fraction of
   the beam (species density), so a hard parapet umbra and a palm's
   sieve shade differently. sfSkyVis doubles as the live query so pawns
   and props standing in a shadow stop casting light they never see. */
// crown occluder: [radius m, crown base m, crown top m, shade density]
function sfCrownOcc(o){
  switch(o.kind){
    case 'sfTree':       return o.big ? [2.7, 2.6, 8.4, 0.85]
                                      : [1.7, 1.6, 5.4, 0.72];
    case 'sfPalm':       return [1.6, 5.0, 7.4, 0.42];
    case 'sfCypress':    return [1.0, 0.3, 7.6, 0.8];
    case 'sfStreetTree': return o.v === 0 ? [2.3, 1.5, 5.2, 0.72]
                                          : [1.2, 1.3, 3.9, 0.55];
    default: return null;
  }
}
/* direct-sun visibility 0..1 at ground point (meters). tMap is the
   per-chunk-bake occluder index (cell -> crown records pre-expanded over
   every cell the disc overlaps); without it we probe the 3x3 prop-cell
   neighborhood — the live query for pawns/props. */
function sfSkyVis(xm, ym, tMap){
  if(SF_SUN.day < 0.05 || isNight()) return 1;
  const cm = SF_M.cell_m, tanEl = Math.tan(Math.max(0.02, SF_SUN.el));
  const z0 = sfElevM(xm, ym);
  const maxD = Math.min(90, 34 / tanEl);
  let vis = 1;
  for(let s = 1.5; s < maxD; s += 1.5){
    const ox = xm + SF_SUN.toX * s, oy = ym + SF_SUN.toY * s;
    const zr = z0 + s * tanEl;
    const bi = sfBldAtM(ox, oy);
    if(bi >= 0 && SF_BLD[bi].hPx / 4.2 + sfElevM(ox, oy) > zr)
      return 0;                                   // opaque mass — umbra
    const cxx = Math.floor(ox / cm), cyy = Math.floor(oy / cm);
    if(tMap){
      const lst = tMap.get(cxx + ',' + cyy);
      if(lst) for(const t of lst){
        const dx = ox - t.x, dy = oy - t.y;
        if(dx * dx + dy * dy < t.r2 && zr > t.zb && zr < t.zt){
          vis *= t.tr; if(vis < 0.04) return 0.04;
        }
      }
    } else {
      for(let ny = -1; ny <= 1; ny++) for(let nx = -1; nx <= 1; nx++){
        const lst = SF_PROP_CELL.get((cxx + nx) + ',' + (cyy + ny));
        if(!lst) continue;
        for(const o of lst){
          const oc = o._occ || (o._occ = sfCrownOcc(o));
          if(!oc) continue;
          const dx = ox - o.x / SF_PXM, dy = oy - o.y / SF_PXM;
          if(dx * dx + dy * dy < oc[0] * oc[0] && zr > oc[1] && zr < oc[2]){
            vis *= 1 - oc[3];
            if(vis < 0.04) return 0.04;
          }
        }
      }
    }
...
/* memoized field query for static props — the sun sector is the cache
   key, so a pawn-free frame costs one compare per prop */
function sfPropVis(o){
  if(o._visQ !== SF_SUN.q){
    o._visV = sfSkyVis(o.x / SF_PXM, o.y / SF_PXM);
    o._visQ = SF_SUN.q;
  }
  return o._visV;
}
/* v84: pavement-level sun visibility for the street-view ground pass —
   the same physics sfSkyVis marches for the top-down bake, solved two
   cheaper ways. Buildings answer through the memoized canyon probe
   (same occluder field the facades already pay for, 4m cells). Tree
   crowns are solved by PROJECTION, not marching: each crown throws a
   shadow disc whose center lands the crown's own height down-sun
   (throw = z·cot(el) along the real solar bearing), grown by the sun's
   ~0.5° penumbra over that throw. Inside the disc the beam is
   crown-transmitted, not zero — a palm sieves (0.42 dense), a cypress
   wallops (0.8) — with a soft rim so the sieve edge reads like real
   scattered shade. Returns 0..1 direct-beam fraction. */
function sfGroundVis(xm, ym){
  if(SF_SUN.day < 0.05 || isNight()) return 1;
  /* the field's own gradients are street-scale — a 10m road, a 12m
     canopy, a 40m parapet throw — so the answer is memoized on an 8m
     frame grid and every tile samples its nearest probe. One frame
     pays ~nc/16 marches instead of nc. */
  const gx8 = Math.round(xm / 8), gy8 = Math.round(ym / 8);
  const k = gx8 * 8192 + gy8;
  const hit = sfLFGet('gvis', k);
  if(hit !== undefined) return hit;
  xm = gx8 * 8; ym = gy8 * 8;
  if(sfCanyonShade(xm, ym, -1) > 0.35) return sfLFSet('gvis', k, 0.04);
  const cm = SF_M.cell_m, tanEl = Math.tan(Math.max(0.02, SF_SUN.el));
  const z0 = sfElevM(xm, ym);
  let vis = 1;
  const cxx = Math.floor(xm / cm), cyy = Math.floor(ym / cm);
  for(let ny = -1; ny <= 1; ny++) for(let nx = -1; nx <= 1; nx++){
    const lst = SF_PROP_CELL.get((cxx + nx) + ',' + (cyy + ny));
    if(!lst) continue;
    for(const o of lst){
      ...
      const r = oc[0],
            zc = (oc[1] + oc[2]) * 0.5 +
                 sfElevM(o.x / SF_PXM, o.y / SF_PXM);
      const thr = (zc - z0) / tanEl;       // shadow throw in meters
      if(thr < -r) continue;               // crown below the probe
      const t2 = Math.max(0, thr);
      const sx = o.x / SF_PXM - SF_SUN.toX * t2,
            sy = o.y / SF_PXM - SF_SUN.toY * t2;
      const rr = r + t2 * 0.009;           // penumbra grows with throw
      const dx = xm - sx, dy = ym - sy, d2 = dx * dx + dy * dy;
      if(d2 >= rr * rr) continue;
      const edge = clamp((rr - Math.sqrt(d2)) / (rr * 0.3), 0, 1);
      vis *= 1 - oc[3] * edge;
      if(vis < 0.04) return sfLFSet('gvis', k, 0.04);
    }
  }
  return sfLFSet('gvis', k, vis);
}
...
/* v82: the memo grew into a heightfield — a Float32Array over the whole
   map at 1m cells, lazily filled on first touch per cell (NaN sentinel),
   evaluated at the cell center. Same function, same shape, but a flat
   array index replaces the Map round-trip: ~110k lookups/frame go from
   hashed-key to pointer math. Max sampling error is half a meter of XY —
   under 4cm of height on the steepest modeled grade (the Dolores bowl),
   far inside the line width of anything it shades. ~10MB resident. */
let SF_ELEV_G = null, SF_ELEV_GW = 0, SF_ELEV_GH = 0;
function sfElevM(xm, ym){
  if(!SF_ELEV_G){
    SF_ELEV_GW = Math.ceil(SF_M.gw * SF_M.cell_m) + 2;
    SF_ELEV_GH = Math.ceil(SF_M.gh * SF_M.cell_m) + 2;
    SF_ELEV_G = new Float32Array(SF_ELEV_GW * SF_ELEV_GH);
    SF_ELEV_G.fill(NaN);
  }
  let ix = Math.floor(xm) + 1, iy = Math.floor(ym) + 1;
  if(ix < 0) ix = 0; else if(ix >= SF_ELEV_GW) ix = SF_ELEV_GW - 1;
  if(iy < 0) iy = 0; else if(iy >= SF_ELEV_GH) iy = SF_ELEV_GH - 1;
  const idx = iy * SF_ELEV_GW + ix;
  let z = SF_ELEV_G[idx];
  if(z === z) return z;                          // NaN = unbaked cell
  xm = ix - 0.5; ym = iy - 0.5;                  // cell center sample
  ...
  SF_ELEV_G[idx] = z;
  return SF_ELEV_G[idx];      // return the STORED value — repeat calls equal
}
...
/* 0..1 soft cloud occlusion over a world point (meters) — sun dimmer.
   v82: evaluates the field on the frame-memoized blob list and caches the
   scalar per 8m cell — the field's own penumbra is ~10x wider than the
   cell, so the memo is inside the noise of the gradient it samples. */
function sfCloudShadow(mx, my){
  const k = Math.round(mx * 0.125) * 1e6 + Math.round(my * 0.125);
  const hit = sfLFGet('cloud', k);
  if(hit !== undefined) return hit;
  const bl = sfShadowBlobs();
  let sh = 0;
  for(let i = 0; i < bl.length; i++){
    const b = bl[i], dx = mx - b[0], dy = my - b[1], d2 = dx * dx + dy * dy;
    if(d2 < b[2] * 4) sh += b[3] * Math.exp(-d2 / b[2]);
  }
  return sfLFSet('cloud', k,
                 Math.min(1, sh * (0.4 + 0.6 * sfCloudCover())));
}
/* v11: ground-projected cloud blobs resolved ONCE per frame — the street
   ...
   v82: the list itself is now memoized on SF_FRAME — sfCloudShadow and
   the painters below share one resolution per rendered frame. */
const SF_BLOB_SH = { fr: -1, a: null };
function sfShadowBlobs(){
  if(SF_BLOB_SH.fr === SF_FRAME && SF_BLOB_SH.a) return SF_BLOB_SH.a;
  ...
  SF_BLOB_SH.fr = SF_FRAME; SF_BLOB_SH.a = out;
  return out;
}
/* ---------------- v47: sunbreaks --------------------------------------
...
const SF_BLOB_GAP = { fr: -1, a: null };
function sfGapBlobs(){
  if(SF_BLOB_GAP.fr === SF_FRAME && SF_BLOB_GAP.a) return SF_BLOB_GAP.a;
  const cs = sfClouds(), n = Math.ceil(cs.length * (0.25 + 0.75 * sfCloudCover()));
  ...
  SF_BLOB_GAP.fr = SF_FRAME; SF_BLOB_GAP.a = out;
  return out;
}
function sfSunGapK(mx, my){
  const k = Math.round(mx * 0.125) * 1e6 + Math.round(my * 0.125);
  const hit = sfLFGet('gap', k);
  if(hit !== undefined) return hit;
  const bl = sfGapBlobs();
  let g = 0;
  for(let i = 0; i < bl.length; i++){
    const b = bl[i], dx = mx - b[0], dy = my - b[1], d2 = dx * dx + dy * dy;
    if(d2 < b[2] * 4) g += b[3] * Math.exp(-d2 / b[2]);
  }
  return sfLFSet('gap', k, Math.min(1, g));
}
/* ---------------- v24: cumulus bodies -------------------------------
...
function sfRainOverlay(cw, ch, slant){
  ...
  ctx.lineWidth = 1;
  const drops = Math.floor((70 + W.rain * 190) * sfQualF());
...
/* ---------------- v34: Karl's leading edge --------------------------
   ...
   the SAME front. */
/* v82: memoized on its INPUTS (tod/hum/wind/cover/rain/storm) rather than
   the frame counter — a harness or game tick that mutates W between
   renders still sees the new value instantly. */
let SF_KARLK_IN = '', SF_KARLK_V = 0;
function sfKarlK(){
  const sig = W.tod + ',' + W.hum + ',' + W.windSpd + ',' + SF_WX.cover +
              ',' + W.rain + ',' + W.storm;
  if(sig === SF_KARLK_IN) return SF_KARLK_V;
  const t = W.tod;
  ...
  SF_KARLK_V = clamp(diur * 0.5 + hum * 0.4 + sfCloudCover() * 0.2 +
               fetch * 0.15 - 0.24, 0, 1);
  SF_KARLK_IN = sig;
  return SF_KARLK_V;
}
...
function sfKarlDepthAt(v){
  const k = Math.round(v * 0.5);                 // v82: 2m cells on the front
  const hit = sfLFGet('kdep', k);
  if(hit !== undefined) return hit;
  ...
  return sfLFSet('kdep', k,
               lim + mod + Math.sin(SF_WX.t * 0.10 + v * 0.013) * 9);
}
...
/* v87: adaptive quality governor — when the smoothed frame cost stays
   above ~26ms the rig steps detail down a level (fewer rain droplets,
   slower PiP feed, coarser lens plate); when it holds under ~12ms it
   steps back up. Touch devices start one level down; ?q=low|med|high
   pins a level and disables the governor. */
const SF_QUAL = { lvl: 0, auto: true, cool: 0 };
(function(){
  try{
    const q = (typeof location !== 'undefined' &&
               (location.search.match(/[?&]q=(low|med|high)/) || [])[1]);
    if(q){ SF_QUAL.lvl = { high: 0, med: 1, low: 2 }[q]; SF_QUAL.auto = false; }
    else if(typeof matchMedia !== 'undefined' &&
            matchMedia('(pointer: coarse)').matches) SF_QUAL.lvl = 1;
  }catch(e){}
})();
function sfQualTick(){
  if(!SF_QUAL.auto) return;
  if(--SF_QUAL.cool > 0) return;
  const old = SF_QUAL.lvl;
  if(SF_PERF.ms > 26 && SF_QUAL.lvl < 2){ SF_QUAL.lvl++; SF_QUAL.cool = 90; }
  else if(SF_PERF.ms < 12 && SF_QUAL.lvl > 0){ SF_QUAL.lvl--; SF_QUAL.cool = 240; }
  if(SF_QUAL.lvl !== old && typeof window !== 'undefined')
    window.dispatchEvent(new Event('resize'));   // dpr cap is level-keyed
}
function sfQualF(){ return [1, 0.55, 0.3][SF_QUAL.lvl]; }
/* ---------------- v82: the light-field cache --------------------------
   Profiling (devtools/perf_v82.js) showed the street pass spending ~40%
   of its JS time re-answering the same three questions ~1.7k times per
   frame: "how much cloud is over this point" (sfCloudShadow), "is this
   point in a sunbreak" (sfSunGapK), "is Karl's sheet overhead"
   (sfKarlShade) — each a ~46-cloud exp() loop per query, plus the canyon
   raymarch and the fog-front interpolant. Every one of those is a SMOOTH
   SCALAR FIELD over world meters: a cloud shadow edge is ~100m wide, the
   fog shade gradient runs 110m, canyon shade changes on facade scale.
   So v82 evaluates each field once per ~8m cell per rendered frame and
   memoizes the rest — the light the sim draws is identical (the error is
   far under one shade gradient), the per-frame work drops by an order of
   magnitude. SF_FRAME ticks at the top of each render pass (sfPerfBegin),
   so a stale cell can never survive a clock tick; the blob lists
   (cloud ground positions projected along the sun vector) are resolved
   once per frame instead of once per query. */
let SF_FRAME = 0;
const SF_LFM = { cloud: { fr: -1, m: new Map() },
                 gap:   { fr: -1, m: new Map() },
                 karl:  { fr: -1, m: new Map() },
                 cany:  { fr: -1, m: new Map() },
                 gvis:  { fr: -1, m: new Map() },   // v84 pavement sun field
                 kdep:  { fr: -1, m: new Map() } };
let SF_LF_HIT = 0, SF_LF_MIS = 0;
function sfLFGet(slot, k){
  const F = SF_LFM[slot];
  if(F.fr !== SF_FRAME){ F.m.clear(); F.fr = SF_FRAME; }
  const v = F.m.get(k);
  if(v === undefined){ SF_LF_MIS++; return undefined; }
  SF_LF_HIT++; return v;
}
function sfLFSet(slot, k, v){ SF_LFM[slot].m.set(k, v); return v; }
...
function sfPerfHud(cw, ch){
  ...
  const lfT = SF_LF_HIT + SF_LF_MIS;
  const lfPct = lfT ? Math.round(100 * SF_LF_HIT / lfT) : 0;
  const lab = `PERF ${fps.toFixed(0)}fps ${ms.toFixed(1)}ms` +
              ` LF${lfPct}% Q${SF_QUAL.lvl}` +
...
function sfTerrChunk(cx, cy, wetQ){
  ...
  for(let iy = 0; iy < CHN; iy++){
    ...
    for(let ix = 0; ix < CHN; ix++){
      ...
      // v80: the flat warm key moved into the sun-field pass below —
      // visibility-gated per half-cell, so shadowed ground stops
      // receiving the gold wash. Fallback only when the field is off
      // (deep overcast / night edges where litA is ~0 anyway).
      if(SF_TERR.litA > 0.004 && (SF_SUN.day <= 0.05 || isNight())){
  ...
  /* v80: sun field — bake the raymarched sky-visibility map into the
     chunk. Two half-cell-resolution layers drawn smoothed (bilinear
     does the penumbra): a warm key where the sun lands (shadowed ground
     keeps only a faint bounce term) and a cool skylight shade where it
     doesn't. One map drives both, so lit and shaded pavement can never
     disagree about where the shadow line runs. */
  if(SF_SUN.day > 0.05 && !isNight()){
    const cm = SF_M.cell_m, SS = 2, N = CHN * SS;
    const tanEl = Math.tan(Math.max(0.02, SF_SUN.el));
    const maxD = Math.min(90, 34 / tanEl);
    // crown occluders: gather veg props over the chunk + the up-sun
    // reach, pre-expanded into every cell each crown disc overlaps
    const reach = Math.ceil(maxD / cm) + 2;
    const ax = Math.round(SF_SUN.toX * reach), ay = Math.round(SF_SUN.toY * reach);
    const xA = Math.min(cx * CHN, cx * CHN + ax) - 2,
          xB = Math.max(cx * CHN + CHN - 1, cx * CHN + CHN - 1 + ax) + 2,
          yA = Math.min(cy * CHN, cy * CHN + ay) - 2,
          yB = Math.max(cy * CHN + CHN - 1, cy * CHN + CHN - 1 + ay) + 2;
    const tMap = new Map();
    for(let cy2 = yA; cy2 <= yB; cy2++) for(let cx2 = xA; cx2 <= xB; cx2++){
      const lst = SF_PROP_CELL.get(cx2 + ',' + cy2);
      if(!lst) continue;
      for(const o of lst){
        const oc = o._occ || (o._occ = sfCrownOcc(o));
        if(!oc) continue;
        const mx = o.x / SF_PXM, my = o.y / SF_PXM;
        const ze = sfElevM(mx, my);
        const rec = { x: mx, y: my, r2: oc[0] * oc[0],
                      zb: oc[1] + ze, zt: oc[2] + ze, tr: 1 - oc[3] };
        const rc = Math.ceil(oc[0] / cm);
        const ccx = Math.floor(mx / cm), ccy = Math.floor(my / cm);
        for(let dy2 = -rc; dy2 <= rc; dy2++)
          for(let dx2 = -rc; dx2 <= rc; dx2++){
            const k2 = (ccx + dx2) + ',' + (ccy + dy2);
            if(!tMap.has(k2)) tMap.set(k2, []);
            tMap.get(k2).push(rec);
          }
      }
    }
    // march one sun ray per half-cell sample
    const vis8 = new Uint8Array(N * N);
    for(let j = 0; j < N; j++){
      const ym = (cy * CHN + (j + 0.5) / SS) * cm;
      for(let i = 0; i < N; i++){
        const xm = (cx * CHN + (i + 0.5) / SS) * cm;
        vis8[j * N + i] = Math.round(sfSkyVis(xm, ym, tMap) * 255);
      }
    }
    const mk = (r, g2, b, f) => {
      const c2 = document.createElement('canvas'); c2.width = c2.height = N;
      const g3 = c2.getContext('2d'), im = g3.createImageData(N, N),
            dd = im.data;
      for(let j = 0; j < N; j++) for(let i = 0; i < N; i++){
        const k2 = (j * N + i) * 4, vis = vis8[j * N + i] / 255;
        dd[k2] = r; dd[k2 + 1] = g2; dd[k2 + 2] = b;
        dd[k2 + 3] = Math.round(255 * f(vis));
      }
      g3.putImageData(im, 0, 0);
      return c2;
    };
    const litA = SF_TERR.litA;
    const shA = clamp(0.40 * Math.min(1, SF_SUN.day * 1.4), 0, 0.44);
    g.imageSmoothingEnabled = true;
    if(litA > 0.003)
      g.drawImage(mk(255, 214, 150, v => litA * (0.3 + 0.7 * v) * 1.35),
                  0, 0, c.width, c.height);
    if(shA > 0.004)
      g.drawImage(mk(34, 48, 92, v => shA * (1 - v)),
                  0, 0, c.width, c.height);
    g.imageSmoothingEnabled = false;
  }
    ...
    if(d.kind === 'bld'){
      ...
      if(o.kind === 'sfCar' && sprC){
        ...
        if(!isNight() && SF_SUN.day > 0.08){
          ...
          sfSoftEllipse(sx + shx, sy + shy,
                        ...
                        0.22 * Math.min(1, SF_SUN.day + 0.3) * sfPropVis(o),
        ...
        if(!isNight() && SF_SUN.day > 0.3 && sfPropVis(o) > 0.4){
      ...
      if(o.kind === 'sfParklet' && sprC){
        ...
        if(!isNight() && SF_SUN.day > 0.08){
          ...
          sfSoftEllipse(sx + shx, sy + shy,
                        cw2 * 0.52 + Math.hypot(shx, shy) * 0.4,
                        chh * 0.5, Math.atan2(shy, shx),
                        0.2 * Math.min(1, SF_SUN.day + 0.3) * sfPropVis(o),
                        sfUmbra(Math.hypot(shx, shy)));
        }
        ctx.drawImage(sprC, sx - cw2 / 2, sy - chh / 2, cw2, chh);
        continue;
      ...
      if(sprC){
        ...
        if(shadeR && !isNight() && SF_SUN.day > 0.08){
          // v80: a crown inside a building's shadow throws nothing —
          // the field owns that pavement now
          const pVis = sfPropVis(o);
          ...
          const aS = 0.26 * Math.min(1, SF_SUN.day + 0.3) * pVis,
        ...
        if(propM && !isNight() && SF_SUN.day > 0.08){
          ...
          sfSoftEllipse(sx + shx, sy + shy,
                        ...
                        Math.atan2(shy, shx),
                        0.24 * Math.min(1, SF_SUN.day + 0.3) * sfPropVis(o),
                        sfUmbra(len));
        }
        ...
        if(vegK && !isNight() && SF_SUN.day > 0.2 && sfPropVis(o) > 0.3){
        ...
        // v80: a crown inside a building's shadow is lit by the sky dome
        // alone — cool and flat, no sun flank, no dapple
        if(vegK && !isNight() && SF_SUN.day > 0.08){
          const pv2 = sfPropVis(o);
          if(pv2 < 0.55){
            ctx.fillStyle = `rgba(44,58,96,${(0.30 * (1 - pv2)).toFixed(3)})`;
            ctx.beginPath();
            ctx.ellipse(sx, sy - ph * 0.55, pw * 0.46, ph * 0.42, 0, 0, Math.PI * 2);
            ctx.fill();
          }
        }
      ...
      ctx.fill();
      // v80: a pawn inside a shadow throws none of its own and takes the
      // same skylight-only fill as the pavement under it
      const pvVis = (!isNight() && SF_SUN.day > 0.08)
        ? sfSkyVis(v.x / SF_PXM, v.y / SF_PXM) : 1;
      if(!isNight() && SF_SUN.day > 0.08 && pvVis > 0.05){
        ...
        sfSoftEllipse(sx + shx, sy + shy,
                      ...
                      Math.atan2(shy, shx),
                      0.28 * Math.min(1, SF_SUN.day + 0.3) * pvVis, sfUmbra(len));
      }
      ...
      ctx.restore();
      // v80: skylight-only body fill — a pawn in a building's shadow
      // reads cool and flat like the pavement under it
      if(pvVis < 0.6){
        ctx.fillStyle = `rgba(44,58,96,${(0.30 * (1 - pvVis)).toFixed(3)})`;
        ctx.beginPath();
        ctx.ellipse(sx, sy - 8 * cam.zoom, 8 * cam.zoom,
                    12 * cam.zoom, 0, 0, Math.PI * 2);
        ctx.fill();
      }
    }
  }
...
function sfStreetWall(b, ei, x1, y1, x2, y2, ex, ey, L, nx, ny, hm, pr, F, night, fwd){
  ...
  if(W.rain > 0.1 && fwd < 90 && !SF_WALL_BAKE)
    sfCorniceDrips(i, ei, x1, y1, ex, ey, L, hm, pr, F);
  ...
  if(det >= 1 && !mural && ny > 0.05 && W.windSpd != null &&
    ...
    sfWashDay(i, ei, x1, y1, ex, ey, L, nx, ny, hm, para, pr, F,
              night, det, isShop, mural, floors, lit, dim, sunK,
              wvx, wvy, ux, uy);
...
function sfCorniceDrips(i, ei, x1, y1, ex, ey, L, hm, pr, F){
...
/* v78: wash-day garnish, extracted so the impostor path can replay
   the live cloth AFTER blitting the baked sprite — the atlas only
   carries static paint; laundry, curtains and cornice pigeons keep
   breathing on top of it. Args mirror the sfStreetWall locals. */
function sfWashDay(i, ei, x1, y1, ex, ey, L, nx, ny, hm, para, pr, F,
                   night, det, isShop, mural, floors, lit, dim, sunK,
                   wvx, wvy, ux, uy){
  const bays = Math.max(1, Math.floor(L / 3.2));
  const quad = (pts, fill) => {
...
/* v78: the atlas used to cap at 150 ENTRIES — but a single street frame
   faces ~570 walls, so every lookup missed and every wall was rebaked
   every frame (the impostor was slower than the vectors it replaced).
   The working set is bounded by PIXELS now (~104MB of RGBA at the cap),
   with a generous entry ceiling as a second belt. Map order is LRU:
   hits re-insert at the tail, eviction always takes the head. */
const SF_WIM_MAX = 1600;
const SF_WIM_PX_MAX = 26e6;
let SF_WIM_PX = 0;
...
function sfWallImpostor(b, ei, x1, y1, x2, y2, ux, uy, L, nx, ny, hm,
  ...
  if(bk){ SF_WIM.delete(key); SF_WIM.set(key, bk); }       // LRU touch
    ...
    SF_WIM_PX += wpx * hpx;
    while(SF_WIM_PX > SF_WIM_PX_MAX || SF_WIM.size >= SF_WIM_MAX){
      const k0 = SF_WIM.keys().next().value,
            v0 = SF_WIM.get(k0);
      SF_WIM_PX -= v0.wpx * v0.hpx;
      SF_WIM.delete(k0);
    }
    SF_WIM.set(key, bk);
  }
...
/* v78: after an impostor blit, replay the camera-live garnish the bake
   deliberately skips (SF_WALL_BAKE-gated): cornice drips in the rain,
   and the wash-day layer — laundry lines, breathing curtains, cornice
   pigeons. Derivation mirrors the top of sfStreetWall so the same
   hashes pick the same garments on sprite and vector paths alike. */
function sfWallLiveOver(b, ei, x1, y1, x2, y2, ex, ey, L, nx, ny, hm,
                        pr, F, night, fwd){
  const i = b.i;
  const isShop = !!b.name || phash(i, 5, 1303) < 0.12;
  const mural = sfMuralWall(i, ei, L, isShop);
...
function sfRenderStreet(cw, ch){
  ...
  let yawOffT = 0;                    // v81: composition pan (follow only)
  if(SF_CAM.director){
    tX = SF_CAM.x; tY = SF_CAM.y; SF_CAM._mate = null;
  } else {
    /* v81: coverage grammar — before the boom math, decide what the shot
       IS. A single keeps the subject on the third opposite their motion
       (look-room — the frame opens where they're headed). A two-shot
       engages when a second pawn is close and facing the subject (or
       standing in a huddle): the lens pans to split the pair and the
       boom widens until both fit with body margin, like an operator
       riding the pedestal for a dialogue line. */
    let mate = null;
    if(v && !v.inBuilding){
      let md = 7.0;
      for(let i2 = 0; i2 < VILLAGERS.length; i2++){
        const pv2 = VILLAGERS[i2];
        if(pv2 === v || pv2.inBuilding) continue;
        const dd = Math.hypot(pv2.x - v.x, pv2.y - v.y) / SF_PXM;
        if(dd >= md) continue;
        const MV = [[0, 1], [0, -1], [-1, 0], [1, 0]][pv2.face] || [0, 1];
        const ux = v.x - pv2.x, uy = v.y - pv2.y, ul = Math.hypot(ux, uy) || 1;
        if((MV[0] * ux + MV[1] * uy) / ul > 0.45 ||
           (!v.moving && !pv2.moving && dd < 5)){ mate = pv2; md = dd; }
      }
    }
    SF_CAM._mate = mate;
    const back0 = Math.max(1.4, SF_CAM._backE == null ? SF_CAM.back : SF_CAM._backE);
    ...
    let clip = sfBoomClip(px, py, -TDX, -TDY, SF_CAM.back, SF_CAM.h);
    if(mate){
      // mate bearing relative to the boom anchor
      const ax = px - TDX * back0, ay = py - TDY * back0;
      const mF = (mate.x / SF_PXM - ax) * TDX + (mate.y / SF_PXM - ay) * TDY;
      const mS = (mate.x / SF_PXM - ax) * TDY - (mate.y / SF_PXM - ay) * TDX;
      if(mF > 1.5){
        const mA = Math.atan2(mS, mF);
        yawOffT = mA * 0.55;                 // split the pair on the axis
        const halfH = Math.atan2(cw * 0.5, Math.max(400, ch * 1.1) * SF_CAM.fov);
        const spread = Math.abs(mA - yawOffT) + Math.atan2(1.5, mF);
        if(spread > halfH * 0.86)            // widen until both fit
          clip = Math.max(clip, Math.min(24, back0 * spread / (halfH * 0.86)));
      }
    } else {
      const lside = SF_CAM._leadX * TDY - SF_CAM._leadY * TDX;
      if(Math.abs(lside) > 0.4)
        yawOffT = -Math.sign(lside) *
          Math.atan2(cw * 0.15, Math.max(400, ch * 1.1) * SF_CAM.fov);
    }
  ...
  // v81: the composition pan glides on its own clock — a partner stepping
  // in or out of the shot reframes over ~0.4s instead of popping
  if(SF_CAM._yawOff == null) SF_CAM._yawOff = yawOffT;
  SF_CAM._yawOff += (yawOffT - SF_CAM._yawOff) * Math.min(1, dtC * 2.4);
  ...
  SF_CAM._syaw = sfAngLerp(SF_CAM._syaw, SF_CAM.yaw + (SF_CAM._yawOff || 0), kRig);
  ...
  /* v81: gust buffeting — wind load on the rig scales with EXPOSURE, not
     altitude on the map: _sh is already height above the local ground,
     so a lens 2m up inside the street canyon barely feels the gust field
     while a crane at 20m over the park takes the full envelope, worst
     broadside (crosswind factor on sin(windAng − yaw)). The slow drift
     term is the operator getting pushed leeward between gusts. At
     street-follow heights expo≈0 so the temporal still-cache keeps
     working on calm sidewalk shots. */
  const bufExpo = clamp((SF_CAM._sh - 2.2) / 14, 0, 1);
  const bufK = (SF_WX.gust || 0) * bufExpo *
               (0.45 + 0.55 * Math.abs(Math.sin((W.windAng || 0) - useYaw)));
  if(bufK > 0.004){
    const tt = SF_WX.t;
    useYaw += (Math.sin(tt * 2.9) * 0.6 + Math.sin(tt * 7.3 + 1.1) * 0.4) * 0.011 * bufK;
    usePitch += Math.sin(tt * 4.7 + 0.6) * 0.007 * bufK;
    SF_CAM._bufRoll = Math.sin(tt * 5.1 + 2.2) * 0.005 * bufK;
    // leeward push: a standing rig slowly weathercocks off the wind axis
    useYaw += Math.sin((W.windAng || 0) - useYaw) * 0.012 * bufK;
  } else SF_CAM._bufRoll = 0;
  SF_LENS.roll = SF_CAM._roll + (SF_CAM._bufRoll || 0);
  ...
  for(let oi = 0; oi < nc; oi++){
    ...
    /* v84: the pavement joins the sun field. Until now the only beam
       blockers this pass knew were clouds and Karl — a cell under a 40m
       parapet shadow still received the warm key underneath the sweep
       pass, and no crown ever sieved light onto the road. sfGroundVis
       answers both: canyon-memoized building umbra + projected crown
       shadow discs. Shade lands as extra skylight-only fill; the warm
       key is gated by the SAME visibility so lit and shade can never
       disagree about where the shadow line runs. */
    let gsv = 1;
    if(litK > 0.02 && cfwd < 150)
      gsv = sfGroundVis(wxm + cm / 2, wym + cm / 2);
    if(gsv < 0.97){
      const qs = Math.min(8, Math.round((1 - gsv) * 0.30 * 24));
      if(qs > 0) qE(shSt[qs], T0, T0 + 2, T0 + 4, T0 + 6);
    }
    // v42: the sun's warm key on whatever the shade leaves lit — grass,
    // asphalt, sidewalk all read gold in open sun, cool where cloud,
    // canyon or Karl already claimed the cell (lit ∝ vis · (1 - csh))
    if(litK > 0.04 && csh < 0.9){
      const ql = Math.min(8, Math.round(litK * gsv * (1 - csh) * (1 + 0.85 * gk) * 8));
    ...
    if(d.k === 'b'){
      ...
      for(let e = 0; e < n; e++){
        ...
        /* v63: impostor atlas — the baked wall sprite re-projects as a
           slice fan; extreme near-plane stays on vectors. v78: rain no
           longer forces the vector pass — the sprite stands and the
           cornice drips replay as a live overlay via sfWallLiveOver,
           which also carries the wash-day cloth the bake can't hold. */
        if(!sfWallImpostor(b, e, x1, y1, x2, y2, ex / L, ey / L, L,
                           nx, ny, hm, pr, night, d.fwd, cw))
          sfStreetWall(b, e, x1, y1, x2, y2, ex, ey, L, nx, ny, hm, pr, F, night, d.fwd);
        else
          sfWallLiveOver(b, e, x1, y1, x2, y2, ex, ey, L, nx, ny, hm,
                         pr, F, night, d.fwd);
  ...
  // v81: director frame guides — thirds grid + center cross, drawn like
  // a field monitor's etched lines. H toggles; follow mode stays clean.
  if(SF_CAM.director && SF_CAM.guides !== false){
    ctx.strokeStyle = 'rgba(235,242,250,0.16)'; ctx.lineWidth = 1;
    ctx.beginPath();
    for(const f of [1 / 3, 2 / 3]){
      ctx.moveTo(cw * f, 0); ctx.lineTo(cw * f, ch);
      ctx.moveTo(0, ch * f); ctx.lineTo(cw, ch * f);
    }
    ctx.stroke();
    ctx.strokeStyle = 'rgba(235,242,250,0.30)';
    ctx.beginPath();
    ctx.moveTo(cw / 2 - 7, ch * 0.42); ctx.lineTo(cw / 2 + 7, ch * 0.42);
    ctx.moveTo(cw / 2, ch * 0.42 - 7); ctx.lineTo(cw / 2, ch * 0.42 + 7);
    ctx.stroke();
  }

  // v9+v81: rig readout — mode, focal length, focus plane, metered EV
  // and compass heading, bottom-left, like a viewfinder data strip
  {
    const mm = Math.round(50 * SF_CAM.fov);
    const mode = SF_CAM.director ? 'DIRECTOR' : (SF_CAM.orbit ? 'ORBIT' : 'FOLLOW');
    const ev = SF_LENS.ev || 0;
    const bearing = ((90 + (SF_CAM._syaw != null ? SF_CAM._syaw : SF_CAM.yaw)
    ...
    const lab = `◉ ${mode}  ${mm}mm  ƒ2.0 · ${Math.round(SF_LENS.foc)}m` +
      `  EV${ev >= 0 ? '+' : ''}${ev.toFixed(1)}  ▲${Math.round(bearing)}°${card}` +
      (SF_CAM.director ? `  h${SF_CAM._sh != null ? SF_CAM._sh.toFixed(1) : SF_CAM.h}m`
                       : (SF_CAM._mate ? '  2-SHOT' : ''));
    ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
  ...
  if(!SF_CAM.director && v && !v.inBuilding){
    ...
    if(rp){
      ...
      // v81: two-shot coverage marks the partner with amber brackets so
      // the operator can read who's in the shot at a glance
      if(SF_CAM._mate){
        const mp = pr(SF_CAM._mate.x / SF_PXM, SF_CAM._mate.y / SF_PXM, 1.0);
        if(mp){
          ctx.strokeStyle = 'rgba(255,205,120,0.7)'; ctx.lineWidth = 1.2;
          const s2 = 11, t2 = 5;
          ctx.beginPath();
          for(const [sx3, sy3] of [[-1, -1], [1, -1], [-1, 1], [1, 1]]){
            ctx.moveTo(mp[0] + sx3 * s2, mp[1] + sy3 * s2 - sy3 * t2);
            ctx.lineTo(mp[0] + sx3 * s2, mp[1] + sy3 * s2);
            ctx.lineTo(mp[0] + sx3 * s2 - sx3 * t2, mp[1] + sy3 * s2);
          }
          ctx.stroke();
        }
      }
...
function sfDollhouse(b, name, cw, ch){
  ...
  /* v85: real floor plans — the deep end of every interior is a second
     room now: a tiled kitchen for the cafés, a stock room for the
     hardware store, a bedroom or kitchen for the flats. Its floor
     finish differs from the front room before the partition goes in. */
  const zS = RM_D - (2.6 + phash(seed, 5, 8500) * 0.9);   // partition depth
  const dgx = -2.1 + phash(seed, 6, 8501) * 4.2;          // doorway lateral
  const bedFlat = !shop && phash(seed, 7, 8502) < 0.45;
  {
    rmRect(-3.5, zS, 3.5, RM_D,
           shop ? 'rgba(196,190,178,0.55)'
         : bedFlat ? 'rgba(80,64,88,0.35)' : 'rgba(126,114,98,0.45)');
    if(shop || !bedFlat){          // kitchen/stock tile grid
      ctx.strokeStyle = 'rgba(60,56,50,0.30)';
      ctx.lineWidth = Math.max(0.5, cam.zoom * 0.8);
      ctx.beginPath();
      for(let z = zS + 0.6; z < RM_D; z += 0.6){
        const a = rmP(-3.5, z), c2 = rmP(3.5, z);
        ctx.moveTo(a[0], a[1]); ctx.lineTo(c2[0], c2[1]);
      }
      for(let x = -3.3; x <= 3.3; x += 0.6){
        const a = rmP(x, zS), c2 = rmP(x, RM_D);
        ctx.moveTo(a[0], a[1]); ctx.lineTo(c2[0], c2[1]);
      }
      ctx.stroke();
    }
  ...
  if(shop){
    ...
    rmRect(-3.5, 5.9, -3.15, Math.min(7.6, zS - 0.15), '#4a3626'); // back-bar shelves
    if(arch === 'taqueria')
      rmRect(-3.3, 4.8, -2.3, Math.min(6.0, zS - 0.15), '#8a2f24'); // salsa bar
    if(arch === 'hardware')
      rmRect(-3.3, 4.2, -2.0, Math.min(7.4, zS - 0.15), '#3c4048'); // aisle shelf
    if(arch !== 'hardware'){
      for(const [tx, tz] of [[-1.7, 4.7], [-0.3, 6.2], [-2.5, 6.9]]){
        if(tz > zS - 0.7) continue;   // v85: tables stay in the front room
    ...
    rmRect(-3.4, 4.4, -2.35, Math.min(6.6, zS - 0.2), '#5a6a58'); // sofa
    rmRect(-3.32, 4.95, -2.45, 5.7, '#c9a86a');
    rmRect(-3.32, 5.75, -2.45, 6.5, '#a85a5a');
    rmRect(2.55, 5.7, 3.42, Math.min(8.2, zS - 0.15), fur); // bookshelf stays front
    rmDisc(-1.2, 5.2, 0.16, '#2c241c');               // floor lamp
    const plZ = Math.min(7.4, zS - 1.5);              // plant stays front of the partition
    rmRect(-3.2, plZ, -2.75, plZ + 0.45, '#8a5a3a');  // potted plant
    rmDisc(-2.97, plZ + 0.2, 0.28, '#3e7a34');
    rmRect(-3.5, 6.9, -3.35, Math.min(7.7, zS - 0.1), '#3a2c1c');   // wall art
    rmRect(-3.5, 7.9, -3.35, Math.min(8.5, zS - 0.1), '#3a2c1c');
    // sewing table by the window (Carmen's flat) / desk for the others
    if(phash(seed, 2, 7100) < 0.5 && zS - 0.3 > 7.9)
      rmRect(0.8, 7.6, 1.9, Math.min(8.2, zS - 0.2), furT);
  }
  /* v85: the partition itself — a wall across the room with a cased
     doorway, then the back room's fixtures behind it. The doorway
     position is the same `dgx` the walk-in view renders on the left
     wall, so both views agree about the same floor plan. */
  {
    const wc = shade(SF_INT_WALL[arch][0], 0.5);
    rmRect(-3.5, zS - 0.07, dgx - 0.48, zS + 0.07, wc);
    rmRect(dgx + 0.48, zS - 0.07, 3.5, zS + 0.07, wc);
    // casing ticks + door leaf swung into the front room
    rmRect(dgx - 0.56, zS - 0.11, dgx - 0.42, zS + 0.11, '#3a2c1c');
    rmRect(dgx + 0.42, zS - 0.11, dgx + 0.56, zS + 0.11, '#3a2c1c');
    {
      const dl0 = rmP(dgx - 0.44, zS), dl1 = rmP(dgx - 0.44 + 0.66, zS - 0.66);
      ctx.strokeStyle = '#4a3626';
      ctx.lineWidth = Math.max(1.2, 2 * cam.zoom);
      ctx.beginPath(); ctx.moveTo(dl0[0], dl0[1]); ctx.lineTo(dl1[0], dl1[1]); ctx.stroke();
    }
    if(shop){
      if(arch === 'hardware'){
        // stock room: shelf rows + boxed goods + a pallet stack
        rmRect(-3.3, zS + 0.4, -0.4, zS + 1.0, '#3c4048');
        rmRect(-3.3, zS + 1.5, -0.4, zS + 2.1, '#3c4048');
        for(let k = 0; k < 8; k++)
          rmRect(-3.2 + k * 0.33, zS + 0.5, -3.05 + k * 0.33, zS + 0.9,
                 ['#b8542e', '#c8a030', '#6a7a8a', '#8a8a80'][k % 4]);
        rmRect(0.4, zS + 0.4, 3.3, zS + 1.6, '#5a5148');
      } else {
        // kitchen: steel prep line on the back wall — range burners,
        // sink, a prep table with plate stacks, steam table (taqueria)
        rmRect(-3.3, RM_D - 0.75, 0.4, RM_D - 0.15, '#8a8f94');
        rmRect(-3.3, RM_D - 0.75, 0.4, RM_D - 0.60, '#b8bcc4');
        for(let k = 0; k < 4; k++)
          rmDisc(-2.9 + k * 0.55, RM_D - 0.45, 0.14, '#2c2c30');
        rmDisc(0.1, RM_D - 0.45, 0.2, '#5a6a72');
        rmRect(1.0, zS + 0.5, 3.2, zS + 1.15, '#7a6a58');
        rmRect(1.0, zS + 0.5, 3.2, zS + 0.62, '#c8b490');
        for(let k = 0; k < 4; k++)
          rmDisc(1.4 + k * 0.5, zS + 0.85, 0.11, ['#e8e0d0', '#d8cba8'][k % 2]);
        if(arch === 'taqueria')
          rmRect(-1.4, zS + 0.5, -0.4, zS + 1.3, '#8a2f24');
      }
    } else if(bedFlat){
      rmRect(-3.35, zS + 0.35, -1.45, zS + 2.25, '#6a4a5a');  // bed
      rmRect(-3.35, zS + 0.35, -1.45, zS + 0.85, '#e8dcc8');  // pillow end
      rmRect(-1.55, zS + 0.35, -1.35, zS + 2.25, '#4a3626');  // footboard
      rmRect(1.6, RM_D - 0.9, 3.3, RM_D - 0.2, '#5a4530');    // dresser
      rmDisc(0.4, zS + 1.3, 0.7, '#8a4a42');                  // bedside rug
    } else {
      // flat kitchen: counter run + range discs + sink, fridge, table
      rmRect(-3.3, RM_D - 0.75, 0.9, RM_D - 0.15, '#7a7268');
      rmRect(-3.3, RM_D - 0.75, 0.9, RM_D - 0.60, '#d8d0c0');
      rmDisc(-1.2, RM_D - 0.45, 0.18, '#4a5a62');
      for(let k = 0; k < 3; k++)
        rmDisc(-2.7 + k * 0.5, RM_D - 0.45, 0.13, '#2c2c30');
      rmRect(1.4, RM_D - 1.6, 2.2, RM_D - 0.2, '#c8ccd0');    // fridge
      rmDisc(2.9, zS + 0.9, 0.45, '#d8cba8');                 // cafe table
      rmRect(2.5, zS + 1.5, 3.3, zS + 1.9, fur);
    }
  }
...
function sfRenderInterior(cw, ch, v){
  ...
  /* v85: the back room shows through a second cased opening on the
     left wall — the same back room the dollhouse draws behind its
     partition (kitchen for the venues, kitchen or bedroom for the
     flats). A real opening: dark jamb, lit far wall, fixture
     silhouettes, warm spill on the floorboards. */
  {
    const dz0 = 2.9, dz1 = 4.0;
    const lit2 = lamps || night;
    rmWallQuad(-1, dz0, dz1, 0, 2.15, '#1a120a');              // jamb dark
    const bg2 = ctx.createLinearGradient(0, RM(-RM_W / 2, dz1, 1.9).y,
                                         0, RM(-RM_W / 2, dz1, 0.12).y);
    bg2.addColorStop(0, lit2 ? '#6a4a24' : '#54432e');
    bg2.addColorStop(1, lit2 ? '#8a5e30' : '#463626');
    rmWallQuad(-1, dz0 + 0.16, dz1 - 0.16, 0.12, 1.9, bg2);    // lit far wall
    // fixture silhouettes inside: counter line + hung pots / shelf line
    rmWallQuad(-1, dz0 + 0.3, dz1 - 0.3, 0.88, 0.98, '#2a1e12');
    for(let k = 0; k < 3; k++)
      rmWallQuad(-1, dz0 + 0.4 + k * 0.3, dz0 + 0.52 + k * 0.3, 1.3, 1.62,
                 '#241a10');
    ctx.strokeStyle = shade('#e8dcc8', 0.8); ctx.lineWidth = 2.5;
    const jp = [RM(-RM_W / 2, dz0, 0), RM(-RM_W / 2, dz1, 0),
                RM(-RM_W / 2, dz1, 2.15), RM(-RM_W / 2, dz0, 2.15)];
    ctx.beginPath(); ctx.moveTo(jp[0].x, jp[0].y);
    for(let i = 1; i < 4; i++) ctx.lineTo(jp[i].x, jp[i].y);
    ctx.closePath(); ctx.stroke();
    if(lit2) rmDisc(-2.9, (dz0 + dz1) / 2, 0.85, 0.01,
                    'rgba(255,200,120,0.15)');                 // spill
  }
