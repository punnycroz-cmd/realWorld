...
/* user-locked rule: businesses on screen are GTA-style parodies, never
   ...
   display layer. integration-1: the canonical parody map + display rules
   live in sf/30a_sf_names.js (world track, world/businesses.md §1/§2) —
   this module must NOT redeclare SF_PARODY_NAMES (shared script scope).
   sfSignName is the signage-styling wrapper over that authority. */
...
function sfSignName(b){
  /* canonical name resolution first (parody map + kind signage + real/
     hidden kind rules, 30a_sf_names.js); signage style is uppercase.
     Falls back to the generic kind sign when the display layer has no
     opinion — preserves the old storefront fallback behavior. */
  const d = (typeof sfDisplayName === 'function')
    ? sfDisplayName(b.name, b.kind) : null;
