...
function sfAgentAct(cid, act){
  ...
  if(verb === 'move'){
    ...
    if(!p) return { ok: false, err: 'unknown place: ' + act.to };
    const cell = sfHomeCell({ poi: p.name });
    v.sfAgent = { verb: 'move', poi: p.name, cell, until: W.tod + holdH };
    return { ok: true, going: p.name };
  }
  if(verb === 'talk'){
