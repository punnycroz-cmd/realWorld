...
function sfInitWorld(){
  ...
  /* ---- v86: the great lawn ----
     Real Dolores Park is one big OPEN southwest-facing slope with the
     canopy massed at the rims (the Church + Dolores allées, the north
     grove by the rec center) — not a forest floor. Cells inside the
     lawn ellipse take no canopy trees at all; the rim band between the
     lawn edge and the park boundary takes them denser, clumped into
     groves. Picnic sites concentrate on the lawn the way the real crowd
     packs the sunny slope. Rect is the same hardcoded convention the
     v17 decals use. */
  const PARK_R = { x0: 250, y0: 118, x1: 360, y1: 304 };
  const pkCx = (PARK_R.x0 + PARK_R.x1) / 2 - 10,
        pkCy = (PARK_R.y0 + PARK_R.y1) / 2 + 15;
  const inParkR = (wx, wy) =>
    wx >= PARK_R.x0 && wx <= PARK_R.x1 && wy >= PARK_R.y0 && wy <= PARK_R.y1;
  const lawnK = (wx, wy) => {   // <1 = inside the open lawn ellipse
    const ex = (wx - pkCx) / 28, ey = (wy - pkCy) / 50;
    return ex * ex + ey * ey;
  };
  ...
  for(let wy = 1; wy < SF_M.gh - 1; wy++){
    for(let wx = 1; wx < SF_M.gw - 1; wx++){
      const t = sfTile(wx, wy);
      const lk = inParkR(wx, wy) ? lawnK(wx, wy) : 9,
            rimBand = inParkR(wx, wy) && lk >= 1 && lk < 2.3;
      if(t === 11 && nStreetTree < 1500){
        ...
        if(h1 > (rimBand ? 0.90 : 0.955) && lk > 1 &&
           !occNear(wx, wy, 3) && !nearPath){
          ...
          nParkVeg++;
        } else if(h1 > 0.99 && lk > 1 &&
                  !occNear(wx, wy, 3) && !doorNear(wx, wy, 2)){
  ...
  for(let wy = 1; wy < SF_M.gh - 1; wy++){
    for(let wx = 1; wx < SF_M.gw - 1; wx++){
      ...
      if(t === 11){
        ...
        /* v86: blankets pack the great lawn — nearly triple the site
           density inside the lawn ellipse, thinning to the old scatter
           everywhere else (a sunny afternoon reads as a patchwork of
           spreads, which is what makes the slope read as a CITY park) */
        const pcCap = (inParkR(wx, wy) && lawnK(wx, wy) < 1) ? 0.30 : 0.115;
        if(phash(wx, wy, 5330) > pcCap) continue;
        if(occNear(wx, wy, 0)) continue;
        const nearPath = sfTile(wx, wy - 1) === 15 || sfTile(wx, wy + 1) === 15 ||
                         sfTile(wx - 1, wy) === 15 || sfTile(wx + 1, wy) === 15;
