...
function updateVillagerBrain(v, dtH){
  if(v.dead) return;
  /* integration-1 fix: this module is the LAST updateVillagerAI binding
     in bundle order, so it must honor the SF dispatch the earlier binding
     (brain/07_ai.js) installed — otherwise the Mission cast's authored
     sfSched routines never execute (latent since the override chain
     existed; harnesses only exercised sfGoTo/sfFollowPath directly). */
