...
var SaveStore = {
  ...
  _ls(){
    ...
    // ?memsave: browser test runs on file:// where the localStorage quota is
    // smaller than a full save blob (~1.2 MB) — fall back to the in-memory
    // backend the node harness uses. Assertions exercise the same logic.
    if(typeof location !== 'undefined' && /[?&]memsave\b/.test(location.search)) return null;
    try{
