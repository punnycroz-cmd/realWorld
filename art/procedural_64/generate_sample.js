/**
 * Test & Export Procedural 64 Character Spritesheet
 */
'use strict';
...
const { Procedural64Generator } = require('./procedural_64_generator.js');
...
for (const cDef of CHARS) {
  ...
  for (let f = 0; f < 4; f++) {
    const frameCanvas = Procedural64Generator.renderFrame(dna, 0, 'idle', f);
    blit(frameCanvas, f, 0);
  }

  // Row 1: Walk Down (6 frames)
  for (let f = 0; f < 6; f++) {
    const frameCanvas = Procedural64Generator.renderFrame(dna, 0, 'walk', f);
    blit(frameCanvas, f, 1);
  }

  // Row 2: Walk Right (6 frames)
  for (let f = 0; f < 6; f++) {
    const frameCanvas = Procedural64Generator.renderFrame(dna, 3, 'walk', f);
    blit(frameCanvas, f, 2);
  }

  // Row 3: Action (6 frames)
  for (let f = 0; f < 6; f++) {
    const frameCanvas = Procedural64Generator.renderFrame(dna, 0, 'profession', f);
  ...
  console.log(`Exported Procedural 64x64 Spritesheet: ${outPath}`);
}
