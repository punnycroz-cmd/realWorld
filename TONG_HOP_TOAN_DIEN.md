...
## 8. Các vấn đề tài liệu đã biết — ĐÃ SỬA HẾT (2026-09-16, theo lệnh user)

> Phát hiện khi biên soạn hồ sơ (2026-09-16). User quyết định: tạm thời không sửa source docs, chỉ ghi nhận ở đây. **Lead sẽ nhắc lại 3 (+1) vấn đề này để sửa sau khi các agent chạy xong toàn bộ Phase 6 (6A→6D).** → Phase 6D commit xong 2026-09-16, user ra lệnh fix → **sửa xong cả 4 trong cùng ngày**, chi tiết bên dưới.

- **8.1. `PROJECT_SUMMARY.md` đã lỗi thời nặng — FIXED 2026-09-16.** Đã viết lại: kiến trúc modular (`src/`, 62 modules, `src/_order.txt`; `scripts/build_willowbrook_natura.py` chỉ là bundler), bundle ~977 KB, "quy tắc sắt" sửa thành "chỉ sửa `src/`, rebuild bundle, không sửa tay HTML"; test suite → 263 dòng, 0 FAIL, PART 12–27 + Examiner Tier-3 audit; git → HEAD `05caf95` (Phase 6D), đã commit, chưa push; bổ sung tóm tắt Phase 2–6 (2A–2F, 3, 4, 5, 6A→6D) và mục 7 "Mô hình nhu cầu canonical". Ngày cập nhật → 2026-09-16.
- **8.2. `GAME_DESCRIPTION.md` lệch roster — FIXED 2026-09-16.** Roster §5: 8 → **9 dân làng**, thêm dòng Clara (thương nhân tơ lụa — khớp `src/data/03_roster.js`); intro "cộng đồng 8 dân làng" → 9. Xóa "Gareth (người can đảm)" khỏi §12 (Gareth không tồn tại trong roster/code); xóa luôn "chó" trong cùng câu vì game không có chó (grep `src/` chỉ ra `doGiveback` trùng chữ). Câu còn lại: "Sợ lửa, sợ đám đông." Cùng lúc xóa "sợ Gareth" trong `PROJECT_SUMMARY.md` §3.
- **8.3. Mâu thuẫn "quần áo ướt" — FIXED 2026-09-16 (đối chiếu code).** Code là chuẩn: `clothingInsul()` tại `src/systems/14a_medical.js:126` — `wetness > 0.4` → `s *= 0.4`, tức **ướt mất 60% khả năng giữ ấm (còn 40%)**. `PROJECT_SUMMARY.md` ghi "60%" đã đúng, giữ nguyên. `GAME_DESCRIPTION.md` ghi "mất hết"/"mất khả năng giữ ấm" là sai → sửa cả 3 chỗ (dòng 41, 97, 263) thành "mất 60% khả năng giữ ấm", dòng 263 ghi rõ công thức + file code.
- **8.4. "8 needs" chưa map chặt vào code — FIXED 2026-09-16 (làm rõ, không bịa).** Kết luận: **không có danh sách "8 needs" chính thức nào trong code.** `brain/utility.js` tính 6 deficit có tên (`satiety, hydration, fatigue, injury, cold, social`) + risk modifiers; body sim theo dõi 7 trạng thái sinh lý (đói/khát/mệt/thân nhiệt/oxy/máu/bệnh). Con số "8" trong `WORK_LOG.md:598` chỉ là số ca chết vì nhu cầu trong một đợt test cũ, không phải danh sách needs. Đã ghi mô hình canonical này vào `PROJECT_SUMMARY.md` §5 mục 7 để sau này Phase 6E/7 có chỗ đối chiếu duy nhất.
